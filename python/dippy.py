...
class dippyClass:
    ...
    def bfdata(self, atom):   # dev PGJ
        ...
        #
        #orgbf=self.bfrd(file,atomnum(atom['name']),ions[0])
        orgbf=self.bfrd(file,ions[0])
        #
        for ion in range(1,len(uion)):
            #new=self.bfrd(file,atomnum(atom['name']),uion[ion])
    ...
    def diplist(aname):
        ...
        for ion in range(1,iel):
            ...
            if atom['ok'] == True:
                ...
                nbf=len(atom['bf'])
                ncbb=len(atom['cbb'])
                print('{:2s}'.format(atom['name']),'{:6s}'.format(dippyClass.roman(ion)),
                    '{:6d}'.format(nlvl),
                      '{:6d}'.format(nbb),
                      '{:6d}'.format(nbf),
                      '{:6d}'.format(ncbb)
