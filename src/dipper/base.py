...
class dipall:
    ...
    ####################################################################################################
    @staticmethod
    def confirmdirs(dipper_imported=False):
    ...
    def renewdirs(self):
        ...
        self.dipper_dbdir= os.environ['XUVTOP']
        self.dipper_spdir= os.path.join( pathlib.Path(self.dipper_dbdir).parent.resolve() , 'spectra' )
        return
        ...
        for im in met:
            ...
            for ic in range(0,nc):    # loop over all coefficients cbf
                ...
                if(csource[ic] == 'shull82' and cion[ic] == ioni and
                   (self.dipper_regime == 'coronal' or nbf ==0)):
    ...
    def bfdata(self, atom):   # dev PGJ
        ...
        #
        self.renewdirs()
        file= os.path.join( self.dipper_dbdir , 'bfsql.db' )
        #
    ...
    def iprd(self):   # nominal 
        ...
        #text = open(  os.path.join( dipall.dipper_dbdir , 'mooreip.dat' )  )
        self.renewdirs()
        text = open(  os.path.join( self.dipper_dbdir , 'mooreip.dat' )  )
        data = text.read()
        text.close()    
        data=data.split()
        #
    ...
    #
        try:
            ...
            self.renewdirs()
            file= os.path.join( self.dipper_spdir , 'lisird_2008255.ascii' )
...
#
    ...
    def regime(self):   # nominal
        #
        # returns the string name of the regime from the global variable dipper_regime
        # 
        r=['coronal','nlte','lte']
        return r[self.dipper_regime] # global variable
...
class diprd(dipall):
    ...
    def __init__(self, atomN,ionnum, boundonly=None, dipper_regime=1, dipper_approx=0, verbose=False):
        ...
        #
        #  output depends on dipper_regime global
        #

        self.atomN = atomN
        self.boundonly = boundonly
        self.dipper_regime = dipper_regime
        self.dipper_approx = dipper_approx
        ...
        self.renewdirs()
        dir=str( self.dipper_dbdir )+ os.path.sep
        #dipall_init = dipall() 
        #dir=str( dipall_init.dipper_dbdir )+ os.path.sep
        #dir=str( dipall.dipper_dbdir )+ os.path.sep
        file='lvlsql.db'
...
class diprd_multi(dipall):    # alpha
    ...
    def __init__(self, atomN,ions, boundonly=True, dipper_regime=1, dipper_approx=0, verbose=False):
        #
...
if __name__ == '__main__':
    ...
    print('Global scope variables all are of the kind dipper_XXX                                          ')
